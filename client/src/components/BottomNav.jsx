import { Home, Utensils, ShoppingBag, User, Phone } from 'lucide-react';
...
const BottomNav = () => {
  ...
  return (
    <div className="md:hidden fixed bottom-6 left-4 right-4 z-[100]">
      {/* Floating Cart Indicator (Swiggy Style) */}
      <AnimatePresence>
        {totalItems > 0 && location.pathname === '/menu' && (
          <motion.button
      ...
      <div className="bg-white/80 backdrop-blur-2xl rounded-[30px] border border-gray-100 shadow-2xl flex items-center justify-around p-3 px-6 shadow-gray-200/50">
        ...
        >
           <div className={`p-2 rounded-2xl relative transition-all ${totalItems > 0 ? 'bg-primary/5 animate-pulse' : ''}`}>
             <ShoppingBag size={20} />
             {totalItems > 0 && (
               <span className="absolute top-1 right-1 bg-primary text-white text-[8px] font-bold h-3.5 w-3.5 rounded-full flex items-center justify-center border border-white">{totalItems}</span>
             )}
           </div>
